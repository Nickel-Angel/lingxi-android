...
public class PhotoPickerActivity extends AppCompatActivity {
  ...
  @Override protected void onCreate(Bundle savedInstanceState) {
    ...
    setContentView(me.cl.lingxi.R.layout.__picker_activity_photo_picker);

    Toolbar mToolbar = (Toolbar) findViewById(me.cl.lingxi.R.id.toolbar);
    setSupportActionBar(mToolbar);
    setTitle(me.cl.lingxi.R.string.__picker_title);
    ...
    if (pickerFragment == null) {
      ...
      getSupportFragmentManager()
          .beginTransaction()
          .replace(me.iwf.photopicker.R.id.container, pickerFragment, "tag")
          .commit();
    ...
    pickerFragment.getPhotoGridAdapter().setOnItemCheckListener(new OnItemCheckListener() {
      @Override public boolean onItemCheck(int position, Photo photo, final int selectedItemCount) {
        ...
        if (selectedItemCount > maxCount) {
          Toast.makeText(getActivity(), getString(me.cl.lingxi.R.string.__picker_over_max_count_tips, maxCount),
              LENGTH_LONG).show();
          return false;
        }
        if(maxCount > 1){
          menuDoneItem.setTitle(getString(me.cl.lingxi.R.string.__picker_done_with_count, selectedItemCount, maxCount));
        }else {
          menuDoneItem.setTitle(getString(me.cl.lingxi.R.string.__picker_done));
        }
        return true;
      }
    });
  ...
  public void updateTitleDoneItem(){
    if(menuIsInflated){
      if(pickerFragment != null && pickerFragment.isResumed()){
        ...
        if(maxCount > 1){
          menuDoneItem.setTitle(getString(me.cl.lingxi.R.string.__picker_done_with_count, size, maxCount));
        }else {
          menuDoneItem.setTitle(getString(me.cl.lingxi.R.string.__picker_done));
        }
  ...
  @Override public boolean onCreateOptionsMenu(Menu menu) {
    if (!menuIsInflated) {
      getMenuInflater().inflate(me.cl.lingxi.R.menu.__picker_menu_picker, menu);
      menuDoneItem = menu.findItem(me.cl.lingxi.R.id.done);
      if (originalPhotos != null && originalPhotos.size() > 0) {
        menuDoneItem.setEnabled(true);
        menuDoneItem.setTitle(
                getString(me.cl.lingxi.R.string.__picker_done_with_count, originalPhotos.size(), maxCount));
      } else {
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    ...
    if (item.getItemId() == me.cl.lingxi.R.id.done) {
